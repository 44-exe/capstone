...
class SerialCom:
    ...
    def read_serial(self):
        if self.serialPort.in_waiting > 0:
            ...
            try:
                ...
                print(f"SERIAL: self.data: {self.data}")
                # self.parse_data()
            except:
                print("Invalid data")
    ...
    def parse_data(self):
        if self.data[0:7] == "OC_vol:":
            self.OC_volt = float(self.data[8:])
            print(f"OC_vol: {self.OC_volt}V", end='  ')
            self.sync += 1
    ...
    def send_arduino(self, duty):
        # duty_str = b''
        # duty_str += struct.pack('f', duty)
        if duty > 90:
            duty = 90
        if duty < 10:
            duty = 10
        # print(f"sending duty to arduino: {round(duty, 2)}")
        string_val = str(round(duty, 2)).encode("ascii") + b'\n'
        print(f"sending duty to arduino: {string_val}")
        self.serialPort.write(string_val)
